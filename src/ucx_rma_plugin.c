...
NCCL_PARAM(UCXAckDelay, "UCXPUT_ACK_DELAY", 0);
...
static nccl_ucp_context_t context = {
    .dev_count = -1
};
                                        ...
                                        void **listen_comm)
{
    ...
    NCCL_STATIC_ASSERT(
       sizeof(nccl_ucp_listen_handle_t) < NCCL_NET_HANDLE_MAXSIZE,
    ...
    NCCLCHECK(ncclSocketInit(&l_comm->sock, &context.if_addr,
                             NCCL_UCP_HANDLE_MAGIC, ncclSocketTypeNetIb,
                             NULL, 1));
    ...
    *listen_comm = l_comm;
    ...
    INFO(NCCL_INIT | NCCL_NET,
         "Listening id=%d dev=%d l_comm=%p", l_comm->id, dev, l_comm);
    return ncclSuccess;
}
...
static ncclResult_t nccl_ucp_context_create(int dev, ucp_context_h *ucp_context)
{
    ucp_params_t params;
    ucp_config_t *config;
...
static nccl_ucp_memh_t *nccl_ucp_mem_register(nccl_ucp_comm_t *comm,
                                              void *data, size_t size, int type)
{
    ...
    mh->mem_type = (type == NCCL_PTR_HOST)?
    ...
    if (status != UCS_OK) {
        WARN("Memory registration failed for comm=%p mem=%p/%zu",
             comm, (void *)addr, size);
        free(mh);
        return NULL;
    }

    status = ucp_rkey_pack(comm->worker->ucp_context, mh->ucp_memh,
                           &mh->rkey_buf, &mh->rkey_buf_size);
    if (status != UCS_OK) {
...
static ncclResult_t nccl_ucp_flush_ep_init(nccl_ucp_comm_t *comm)
{
    ...
    UCXCHECK(ucp_ep_create(comm->worker->ucp_worker, &params,
                           &comm->ucp_flush_ep));
    free(attr.address);
    return ncclSuccess;
}

static nccl_ucp_comm_t *nccl_ucp_comm_create(int dev, nccl_ucp_req_type_t type)
{
    ...
    comm->local.share_mh = nccl_ucp_mem_register(comm, &comm->local.share,
                                                 sizeof(comm->local.share),
                                                 NCCL_PTR_HOST);
                                 ...
                                 ncclNetDeviceHandle_t **sendDevComm)
{
    ...
    *send_comm = NULL;
    ...
    case NCCL_UCP_START:
        ...
        NCCLCHECK(ncclSocketInit(&stage->comm->sock,
                                 &handle->listener.addr,
                                 handle->magic,
    ...
    case NCCL_UCP_CONNECT:
        ...
        memcpy(peer.address, comm->worker->address,
               comm->worker->address_length);
    ...
    case NCCL_UCP_RECEIVE_REMOTE:
        ...
        *send_comm    = comm;
        ...
        INFO(NCCL_INIT | NCCL_NET,
             ...
             "ack_delay=%d ack_skip=%d", comm,
             comm->peer.comm, handle->listener.id,
             ncclParamUCXAckDelay(), ncclParamUCXAckSkip());
        break;
                                 ...
                                 ncclNetDeviceHandle_v7_t **recvDevComm)
{
    ...
    *recv_comm = NULL;
    ...
    case NCCL_UCP_START:
        ...
        NCCLCHECK(ncclSocketInit(&comm->sock, NULL,
                                 NCCL_UCP_HANDLE_MAGIC,
                                        ...
                                        int *inflight)
{
    ...
    assert(
       (rva >= (void *)comm->remote.share) && (rva + size) <=
    ...
    status_ptr = ucp_put_nbx(comm->ucp_ep, va, size,
                             (uint64_t)rva, comm->remote.rkey, &param);
                                       ...
                                       int type, void  **mhandle)
{
    ...
    if (mh) {
        ...
        if (comm->gpu_flush) {
            UCXCHECK(ucp_ep_rkey_unpack(comm->ucp_flush_ep,
                                        mh->rkey_buf, &mh->rkey));
        }
    }
                                       ...
                                       void **request)
{
    ...
    *request = NULL;
    ...
    status = nccl_ucp_shared_put(
                 comm,
                 rtr,
                 sizeof(*rtr) - (NCCL_UCP_MAX_RECV - n) * sizeof(*rtr->chunk),
                 remote,
                 &req->inflight);
                                    ...
                                    unsigned short rkey_id)
{
    ...
    if (nccl_rkey->rkey_id != rkey_id) {
        ...
        status = ucp_ep_rkey_unpack(comm->ucp_ep,
                                    packed->rkey_buf,
                                    &nccl_rkey->rkey);
        if (status != UCS_OK) {
            return NULL;
...
static ncclResult_t nccl_ucp_send(nccl_ucp_comm_t *comm,
                                  unsigned short id,
                                  unsigned i,
                                  void *data,
                                  int size,
                                  ...
                                  void **request)
{
    ...
    *request = NULL;
    ...
    status_ptr = ucp_put_nbx(comm->ucp_ep, data, size,
                             rtr->chunk[i].data,
                             rkey, &param);
    ...
    for (id = comm->rtr_id;; id++) {
        ...
        for (i = 0; i < rtr->count; i++) {
            if (rtr->chunk[i].tag == tag) {
                result = nccl_ucp_send(comm, id, i, data, size,
                                       mhandle, request);
                goto out;
            }
        }
                                        ...
                                        void **request)
{
    ...
    status_ptr = ucp_get_nbx(comm->ucp_flush_ep,
                             &comm->local.share.dummy_mem,
                             1, (uint64_t)data[last], mh[last]->rkey,
                             &param);
    ...
    *done = 0;
    ...
    if (req->type == NCCL_UCP_TYPE_ISEND) {
        ...
        *done = (req->inflight == 0) && ((atp->reqs > 1) || (rtr->avail < 0));
        ...
        __sync_synchronize();
        *done = (req->inflight == 0) &&
                (atp->id_start == req->rtr_id) && (atp->id == req->rtr_id) &&
